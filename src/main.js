...
import {replacementAmount} from './substitutions.js';
...
function renderMeal(m,index) {
 const {have,missing,percent,substitutions,covered} = matchDetails(m);
 const mainHave = [...new Set([...have, ...substitutions.map(s=>s.replacement)])].filter(id => !STAPLES.includes(id));
 return `<article class="meal-card" data-meal="${m.id}"><div class="meal-count">${String(index+1).padStart(2,'0')}</div><div class="meal-main"><div class="meal-title"><div><p>${m.mealType} · ${m.cuisine}</p><h3>${escape(m.name)}</h3><span>${m.difficulty} · ${m.needed.length} ingredients</span></div><span class="match-badge ${missing.length?'':'complete'}">${percent}% ${substitutions.length?'with swaps':'available'}</span></div>${tags(m)}<div class="coverage"><div><b>${substitutions.length?'Covered':'You have'} ${covered}/${m.needed.length} ingredients</b><span>${missing.length?`${missing.length} missing`:substitutions.length?'Ready with substitutions':'Ready to cook'}</span></div><div class="progress"><i style="width:${percent}%"></i></div></div><div class="meal-ingredients"><p>Using <span>${mainHave.map(id=>ingredient(id).name).join(' · ') || (have.length?'Your cupboard essentials':'Nothing from your kitchen yet')}</span></p>${missing.length?`<p class="missing">Need <span>${missing.map(id=>ingredient(id).name).join(' · ')}</span></p>`:`<p class="all-set">✓ ${substitutions.length?'You can cook this with the swaps below':'You have everything'}</p>`}</div>${renderSubstitutions(m,true)}${videoLinks(m,true)}</div><div class="meal-actions"><a class="cook-button" href="#/recipe/${m.id}">View recipe →</a></div></article>`;
}
...
function renderResults() {
 ...
 const exact=meals.filter(m=>matchDetails(m).exact);
 const swapped=meals.filter(m=>{const match=matchDetails(m);return match.ready && !match.exact;});
 const partial=meals.filter(m=>matchDetails(m).missing.length);
 const section=(label,title,description,items,offset)=>`<section class="match-section" aria-label="${label}"><h3>${title} · ${items.length}</h3><p>${description}</p><div class="meal-list">${items.map((m,i)=>renderMeal(m,offset+i)).join('')}</div></section>`;
 return `<p class="result-count" role="status">Showing ${meals.length} of ${recipes.length} recipes · ${exact.length} exact matches · ${swapped.length} ready with swaps</p>${meals.length ? `${exact.length?section('100% matches','Exact matches','Every listed ingredient is in your kitchen, including basics and spices.',exact,0):'<p class="no-exact">No exact matches for these selections. Suitable substitutions are shown below when available.</p>'}${swapped.length?section('Ready with substitutions','Ready with substitutions','All ingredients are covered using the swaps listed on each recipe. Read the quantities and preparation notes before cooking.',swapped,exact.length):''}${!exact.length&&!swapped.length?'<div class="no-matches"><h3>No fully covered recipes yet</h3><p>Check your cupboard or the missing ingredients below. We only suggest suitable replacements you have selected.</p></div>':''}${partial.length?section('Recipes with missing ingredients','Needs more ingredients','These recipes still need the listed ingredients after suitable substitutions.',partial,exact.length+swapped.length):''}`:'<div class="no-matches"><h3>No recipes match that search</h3><p>Try a dish, cuisine, or meal type.</p></div>'}`;
}
